...
func insertionSort1(n: Int, arr: [Int]) -> Void {
    ...
    let valueToStore = arr[n - 1]
    var i = n - 1
    var stringOfInts = ""
    
    while i >= 0 {
        if i - 1 >= 0 && mutableArray[i - 1] > valueToStore {
            mutableArray[i] = mutableArray[i - 1]
            i -= 1

            stringOfInts = ""
            for number in 0...(n - 2) {
                stringOfInts.append("\(mutableArray[number]) ")
            }
            stringOfInts.append(String(mutableArray[n - 1]))
            
            print(stringOfInts)
        } else {
            mutableArray[i] = valueToStore
...
//insertionSort1(n: 5, arr: [1,2,4,5,3])
